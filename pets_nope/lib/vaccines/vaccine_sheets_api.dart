import 'vaccines.dart';
import 'package:gsheets/gsheets.dart';

class VaccineSheetsApi {
  static const _credentials = r'''
  {
  "type": "service_account",
  "project_id": "petsnope",
  "private_key_id": "a52b130d4b6002e97f28ab6a69139ecd5cc246bb",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n66zQIwwd5QQunEcG6xmbq/2s\n-----END PRIVATE KEY-----\n",
  "client_email": "gsheets@petsnope.iam.gserviceaccount.com",
  "client_id": "116299565951311570395",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/gsheets%40petsnope.iam.gserviceaccount.com"
}
  ''';
  static final _spreadsheetId = '1p0bTR4bAfdLdslhtFk3UbCLWKyfRBHWB3bvuDCTNcS4';
  static final _gsheets = GSheets(_credentials);
  static Worksheet? _vaccineSheet;

  static Future init() async {
    try {
      final spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _vaccineSheet = await _getWorkSheet(spreadsheet, title: 'vaccines');

      final firstRow = VaccineFields.getFields();
      _vaccineSheet!.values.insertRow(1, firstRow);
    } catch (e) {
      print('Init Error: $e');
    }
  }

  static Future<Worksheet> _getWorkSheet(
      Spreadsheet spreadsheet, {
        required String title,
      }) async {
    try {
      return await spreadsheet.addWorksheet(title);
    } catch (e) {
      return spreadsheet.worksheetByTitle(title)!;
    }
  }

  static Future<int> getRowCount() async {
    if (_vaccineSheet == null) return 0;

    final lastRow = await _vaccineSheet!.values.lastRow();
    return lastRow == null ? 0 : int.tryParse(lastRow.first) ?? 0;
  }

  static Future<List<Vaccine>> getAll() async {
    if (_vaccineSheet == null) return <Vaccine>[];

    final vaccines = await _vaccineSheet!.values.map.allRows();
    return vaccines == null ? <Vaccine>[] : vaccines.map(Vaccine.fromJson).toList();
  }
  static Future<List<Vaccine>> getByFk(int fk) async {
    if(_vaccineSheet == null) return <Vaccine>[];

    final vaccines = await _vaccineSheet!.values.map.allRows(mapTo: fk);
    return vaccines == null ? <Vaccine>[] : vaccines.map(Vaccine.fromJson).toList();
  }
  static Future<Vaccine?> getById(int id) async {
    if (_vaccineSheet == null) return null;

    final json = await _vaccineSheet!.values.map.rowByKey(id, fromColumn: 1);
    return json == null ? null : Vaccine.fromJson(json);
  }

  static Future insert(List<Map<String, dynamic>> rowList) async {
    if (_vaccineSheet == null) return;

    _vaccineSheet!.values.map.appendRows(rowList);
  }

  static Future<bool> update(
      int id,
      Map<String, dynamic> vaccine,
      ) async {
    if (_vaccineSheet == null) return false;

    return _vaccineSheet!.values.map.insertRowByKey(id, vaccine);
  }

  static Future<bool> updateCell({
    required int id,
    required String key,
    required dynamic value,
  }) async {
    if (_vaccineSheet == null) return false;

    return _vaccineSheet!.values.insertValueByKeys(
      value,
      columnKey: key,
      rowKey: id,
    );
  }

  static Future<bool> deleteById(int id) async {
    if (_vaccineSheet == null) return false;

    final index = await _vaccineSheet!.values.rowIndexOf(id);
    if (index == -1) return false;

    return _vaccineSheet!.deleteRow(index);
  }
}
